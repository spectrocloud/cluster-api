...
func (webhook *KubeadmControlPlane) ValidateUpdate(_ context.Context, oldK, newK *controlplanev1.KubeadmControlPlane) (admission.Warnings, error) {
	...
	allowedPaths := [][]string{
		...
		// Spectro fork (a2e760c0 "relax kcp webhook"): replace the upstream
		// per-field spec.kubeadmConfigSpec allowlist with a single catch-all so
		// ANY field under spec.kubeadmConfigSpec may be modified on update.
		// The catch-all subsumes target v1beta2 additions (timeouts, bootCommands,
		// certificateValidityPeriodDays, encryptionAlgorithm, format, mounts, ...).
